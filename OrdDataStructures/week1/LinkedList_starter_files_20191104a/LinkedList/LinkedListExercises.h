...
void LinkedList<T>::insertOrdered(const T& newData) {
  ...
  if (getHeadPtr() == NULL) {
    pushFront(newData);
  } else if (newData < getHeadPtr()->data) {
    pushFront(newData);
  } else if (newData > getTailPtr()->data) {
    pushBack(newData);
  } else {
    Node* nextNode;
    Node* currNode = getHeadPtr();
    const Node* lastNode = getTailPtr();
    while (currNode != lastNode) {
      // std::cout << "Entered while loop" << std::endl;
      nextNode = currNode->next;
      currNode = nextNode;
      if (newData < currNode->data) {
        // new Data should be inserted before nextNode
        // std::cout << "Found node to insert" << std::endl;
        Node* prev = currNode->prev;
        Node* next = currNode->next;
        
        // std::cout << "Prev Value:"<< prev->data << std::endl;
        // std::cout << "this Value:"<< newData << std::endl;
        // std::cout << "Next Value:"<< currNode->data << std::endl;

        // std::cout << "Assigning new node" << std::endl;
        Node* thisNode = new Node(newData);
        // std::cout << "setting pointers" << std::endl;
        (*thisNode).prev = prev;
        (*thisNode).next = currNode;
        (*prev).next = thisNode;
        (*currNode).prev = thisNode;
        currNode = getTailPtr();
        size_++;
      }
    }
...
LinkedList<T> LinkedList<T>::merge(const LinkedList<T>& other) const {
  ...
  while (!left.empty() || !right.empty()) {
    ...
    if (right.getHeadPtr() == NULL && left.getHeadPtr() != NULL) {
      merged.pushBack(left.front());
      left.popFront();
    } else if (right.getHeadPtr() != NULL && left.getHeadPtr() == NULL) {
