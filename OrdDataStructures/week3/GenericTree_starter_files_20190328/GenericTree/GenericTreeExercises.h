...
static void treeFactory(GenericTree<int>& tree) {
  ...
  //Check tree is empty
  if (tree.getRootPtr() != NULL) {
    tree.clear();
  }
  std::vector<int> elements = {4, 8, 15, 42, 23, 15};
  GenericTree<int>::TreeNode *child;
  GenericTree<int>::TreeNode  *parent = tree.createRoot(elements[0]);
  GenericTree<int>::TreeNode *child8 = parent->addChild(8);
  GenericTree<int>::TreeNode *child15 = parent -> addChild(15);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  std::vector<TreeNode*> levelChildren = rootNodePtr->childrenPtrs;
  std::vector<TreeNode*> thisLevelChildren;
  std::vector<TreeNode*> nextLevelChildren;
  std::stack<T> levelItems;;
  while (!levelChildren.empty()) {
    for (std::size_t it = 0; it < levelChildren.size(); ++it)
    {
      TreeNode* last = levelChildren[it];
      results.push_back(last -> data);
      thisLevelChildren = last->childrenPtrs;
      for (std::size_t i = 0; i < thisLevelChildren.size(); ++i)
      {
        TreeNode* d = thisLevelChildren[i];
        nextLevelChildren.push_back(d);
      }
    }
